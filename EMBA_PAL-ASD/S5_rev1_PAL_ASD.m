...
plotAggLogRTsModel(modSpaceNew, 'pilots', revDir, false, true, pilot.data) % [!ADJUSTED]
...
save(fullfile(revDir, 'modSpace.mat'), 'modSpaceSel', 'modSpaceAll',...
    'modSpaceRobMean', 'modSpaceHGF', 'modSpaceNew')

% now we check the model identifiability
checkModelIdentifiability(nSim, modSpaceSel, revDir);
plotModelIdentifiability(modSpaceSel, revDir);
...
writetable(tbl, fullfile(revDir, 'BOpars.csv'))


%% Perform posterior predictive checks for RW model

% Plot simulated data in comparison to real subject data for RW model
plotLogRTsModel(modSpaceNew, 'main', revDir, 0, 1);

% Plot many simulated datasets in comparison to real subject data for RW model
% focusing on the best and the worst fit participants in the HGF model
plotMultiLogRTsModel(modSpaceNew, 'main', revDir, ...
    [38, 89, 77, 58, 36, 73, 45, 51], 1, nSim, true);

% Create simulated data for better plotting in R
simPostRT(modSpaceNew, 'main', revDir, 1, nSim);

% Plot predicted differences between difficulty & expectedness [!HARDCODED!]
plotAggLogRTsModel(modSpaceNew, 'main', revDir, false, true, data(end).u)
plotAggLogRTsModelPhases(modSpaceHGF, 'main', saveDir, data(end).u)
