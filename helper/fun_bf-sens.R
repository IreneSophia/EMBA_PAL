...
bf_sens_2int = function(m.orig, pred1, pred2, pr.desc, main.code, file.log, sense_dir, iter = 1000, reps = 1) {
  ...
  write(sprintf("%s:\t%s * %s", now(), pred1, pred2), file.log, append = TRUE)
...
# 2 predictors + 1 covariate ----------------------------------------------

bf_sens_2int1cov = function(m.orig, pred1, pred2, cov, pr.desc, main.code, file.log, sense_dir, iter = 1000, reps = 1) {
  ...
  write(sprintf("\n---------------------------------------\n%s:\t%s\n", now(), pr.desc), file.log, append = TRUE)
  write(sprintf("%s:\t%s * %s + %s", now(), pred1, pred2, cov), file.log, append = TRUE)
  # determine the priors
  if (pr.desc == "chosen") {
    # extract the priors
    priors = m.orig$prior
  ...
  write(sprintf("1,%.4f,%s",
                0,
                pr.desc), file.out, append = T)
  write(sprintf("%s * %s + %s,%.4f,%s", pred1, pred2, cov,
                bf,
                pr.desc), file.out, append = T)
  ...
  fixed = c(
    ...
    cov,
    sprintf('%s + %s', pred1, pred2),
    sprintf('%s + %s', pred1, cov),
    sprintf('%s + %s', pred2, cov),
    sprintf('%s + %s + %s', pred1, pred2, cov),
    sprintf('%s + %s + %s:%s', pred1, pred2, pred1, pred2),
    sprintf('%s + %s + %s + %s:%s', pred1, pred2, cov, pred1, pred2)
  )
...
bf_sens_fixed = function(m.orig, fixed, pr.desc, main.code, file.log, sense_dir, iter = 1000, reps = 1) {
  ...
  write(sprintf("\n---------------------------------------\n%s:\t%s\n", now(), pr.desc), file.log, append = TRUE)
  write(sprintf("%s:\tstart with predefined models", now()), file.log, append = TRUE)
  # determine the priors
  if (pr.desc == "chosen") {
    # extract the priors
    priors = m.orig$prior
  ...
  # LOOP THROUGH ALL THE PREDETERMINED MODELS
