...
def PAL_pupilpreprocess(et_file, beh_file, out,
                        plotting=True, margin=10, lp_filter=50):
    ...
    df_trl = pd.read_csv(os.path.split(beh_file)[0] + '/PAL_scheme-pic.csv', 
    ...
    print(subID, "before artefact rejection:", str(round(np.mean(pupil == 0), 2)))
    ...
    if lp_filter != None:
        ...
        pupil = srs.filter_lowpass(df_temp['pupil'], freq_max = 50, 
                                   sampling_freq = 500)
    ...
    # find all the NAs after smoothing
    idx = np.where(np.isnan(pupil))[0]
    ...
    dm = DataMatrix(length=336)
    dm.subID = df_beh['subID']
    dm.trl   = df_beh['trl']
    dm.key   = df_beh['key']
    dm.rts   = df_beh['rt']
    dm.exp   = df_beh['expected']
    dm.pupil = MultiDimensionalColumn(shape=(1050,))
    dm.artft = 0
    dm.miss  = 0
    ...
    for index, row in df_idx.iterrows():
        ...
        dm.pupil[trl - 1] = df_pup.loc[(index-150):(index+899),'pupil']
        dm.artft[trl - 1] = np.mean(df_pup.loc[(index-150):(index+899),'miss'])
