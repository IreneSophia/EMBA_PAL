function [] = simModelsAppend(modSpace, nSim, u, saveDir, revDir, verbose, maxRep)
...
if ~exist(fullfile(revDir, 'sim'), 'dir')
   mkdir(fullfile(revDir, 'sim'))
end
load(save_path, 'sub');
sim.sub = [sub sim.sub];
save(fullfile(revDir, 'sim', ['simulated_data']), '-struct', 'sim');
