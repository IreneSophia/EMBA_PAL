...
if (!file.exists("data/PAL_data.RData")) {
  ...
  df.recode = df.tsk %>% select(PID, subID, diagnosis) %>% distinct()
  recode = as.character(df.recode$subID)
  names(recode) = df.recode$PID
  
  # save this information to use it with pupil sizes
  write_csv(df.recode, file.path(dt.path, "PID_anonymisation.csv"))
  
  # remove the PID from the dataframe  
...
# COMP < BOTH
e1 = hypothesis(m.var, "0 < diagnosis1 + diagnosis2 + 2*diagnosis3", 
                alpha = 0.025)
e1

# COMP < ASD
e2 = hypothesis(m.var, "0 < diagnosis1 + 2*diagnosis2 + diagnosis3", 
                alpha = 0.025)
e2
...
as.data.frame(t(df.des %>% 
    ...
    map_df(~ diff(.x)))) %>%
  filter(V1 != 0)
e3 = hypothesis(m.var, "0 < 2*diagnosis1:phase1 + diagnosis2:phase1 + diagnosis3:phase1 +
                - (2*diagnosis1:phase2 + diagnosis2:phase2 + diagnosis3:phase2)", 
                alpha = 0.025)
e3

# COMP(vol-pre) > ASD(vol-pre)
e4 = hypothesis(m.var, "0 < diagnosis1:phase1 + 2*diagnosis2:phase1 + diagnosis3:phase1 +
                - (diagnosis1:phase2 + 2*diagnosis2:phase2 + diagnosis3:phase2)", 
                alpha = 0.025)
e4

# COMP(vol-pre) > BOTH(vol-pre)
e5 = hypothesis(m.var, "0 < diagnosis1:phase1 + diagnosis2:phase1 + 2*diagnosis3:phase1 +
                - (diagnosis1:phase2 + diagnosis2:phase2 + 2*diagnosis3:phase2)", 
                alpha = 0.025)
e5
...
as.data.frame(t(df.des %>% 
  ...
  filter(V1 != 0) 
e6 = hypothesis(m.var, "0 > 2*diagnosis1:phase1 + diagnosis2:phase1 + diagnosis3:phase1 +
                + 2*(2*diagnosis1:phase2 + diagnosis2:phase2 + diagnosis3:phase2)", 
               alpha = 0.025)
e6

# COMP(post-vol) < ASD(post-vol)
e7 = hypothesis(m.var, "0 > diagnosis1:phase1 + 2*diagnosis2:phase1 + diagnosis3:phase1 +
                + 2*(diagnosis1:phase2 + 2*diagnosis2:phase2 + diagnosis3:phase2)", 
                alpha = 0.025)
e7

# COMP(post-vol) < BOTH(post-vol)
e8 = hypothesis(m.var, "0 > diagnosis1:phase1 + diagnosis2:phase1 + 2*diagnosis3:phase1 +
                + 2*(diagnosis1:phase2 + diagnosis2:phase2 + 2*diagnosis3:phase2)", 
                alpha = 0.025)
e8
...
df.ms = df.ms %>%
  mutate(
    ...
    ASDvol        = rowMeans(across(matches("ASD_volatile.*"))),
    ASDpre        = rowMeans(across(matches("ASD_prevolatile.*"))),
    BOTHvol       = rowMeans(across(matches("BOTH_volatile.*"))),
    BOTHpre       = rowMeans(across(matches("BOTH_prevolatile.*"))),
    e3_COMPvADHD  = (COMPvol - COMPpre) - (ADHDvol - ADHDpre),
    e4_COMPvADHD  = (COMPvol - COMPpre) - (ASDvol  - ASDpre),
    e5_COMPvADHD  = (COMPvol - COMPpre) - (BOTHvol - BOTHpre)
    )

psych::describe(df.ms %>% select(starts_with("e") | starts_with("h")))

```

The model supports our hypothesis regarding increased reaction time variance in adults with ADHD compared to adults without any psychiatric diagnoses (*estimate* = `r round(h1a$hypothesis$Estimate,2)` [`r round(h1a$hypothesis$CI.Lower,2)`, `r round(h1a$hypothesis$CI.Upper,2)`], *posterior probability* = `r round(h1a$hypothesis$Post.Prob*100,2)`%). Specifically, the model predicts a reaction time variance of `r round(mean(df.ms$COMP), 2)`ms [`r round(ci(df.ms$COMP)$CI_low, 2)`, `r round(ci(df.ms$COMP)$CI_high, 2)`] in COMP adults, of `r round(mean(df.ms$ADHD), 2)`ms [`r round(ci(df.ms$ADHD)$CI_low, 2)`, `r round(ci(df.ms$ADHD)$CI_high, 2)`] in adults with ADHD, of `r round(mean(df.ms$ASD), 2)`ms [`r round(ci(df.ms$ASD)$CI_low, 2)`, `r round(ci(df.ms$ASD)$CI_high, 2)`] in adults with ASD and of `r round(mean(df.ms$BOTH), 2)`ms [`r round(ci(df.ms$BOTH)$CI_low, 2)`, `r round(ci(df.ms$BOTH)$CI_high, 2)`] in adults with ADHD and ASD. 
...
for (pr.desc in pr.descriptions) {
  tryCatch({
    ...
    bf_sens_3int(m.var.bf, "diagnosis", "phase", "expected", 
                 ...
                 file.path("logfiles", "log_PAL-var_bf.txt"), # log file
...
df.best = df.var.bf %>%
 ...
 filter(bf.rank <= 5)
best = df.best$`population-level`
df.var.bf %>%
  filter(`population-level` %in% best) %>%
  mutate(
    sd = as.factor(case_when(
      priors == "chosen" ~ "1",
      substr(priors, 1, 3) == "sdx" ~ gsub("sdx", "", priors),
...
kable(head(df.var.bf %>% filter(priors == "chosen") %>% select(-priors) %>%
  filter(`population-level` != "1") %>% arrange(desc(bf.log)) %>% 
        mutate(
...
`` `r "rt.cor ~ diagnosis * expected * phase * difficulty + (expected * phase * difficulty | subID) + (diagnosis * expected * phase * difficulty | trl)"` ``


`` `r "rt.cor ~ diagnosis * expected * phase + (expected * phase | subID) + (diagnosis * expected * phase | trl)"` ``
                   
Unfortunately, they both showed suboptimal posterior fit. They are uploaded to GitHub and can be inspected. Therefore, we aggregated the reaction times for correct trials using the median (see above in the preparation of the data). 
...
After having run the SBC, we realised that setting the threshold to Gaussian with a mean of 100ms does not fit well with a model based on aggregated reaction times - they are unlikely to be this low. Therefore, we change the mean of this one prior, but assume that the SBC still holds. 
...
df.des = cbind(df.pal, 
               model.matrix(~ diagnosis * expected * phase * difficulty, 
                            data = df.pal)) %>%
  select(-subID, -rt.cor, -rt.use, -acc) %>% distinct()
...
h1b = hypothesis(m.pal, "0 < 
                 diagnosis1:expected1 + 2*diagnosis2:expected1+ diagnosis3:expected1")
h1b
...
h1c = hypothesis(m.pal, "0 > 
                 2*diagnosis1:expected1 + diagnosis2:expected1+ diagnosis3:expected1", 
                 alpha = 0.025)
h1c

# H1d: COMP(volatile-prevolatile)) < ASD(volatile-prevolatile)
as.data.frame(t(df.des %>% 
    filter((diagnosis == "ASD" | diagnosis == "COMP") & phase != "postvolatile") %>%
    group_by(diagnosis, phase) %>%
    summarise(across(where(is.numeric), ~ mean(.x))) %>%
    group_by(diagnosis) %>%
...
h1d = hypothesis(m.pal, 
                 "(diagnosis1:phase1 + 2*diagnosis2:phase1 + diagnosis3:phase1) -
                  (diagnosis1:phase2 + 2*diagnosis2:phase2 + diagnosis3:phase2) <
                  0")
h1d
...
as.data.frame(t(df.des %>% 
    filter((diagnosis == "ASD" | diagnosis == "COMP") & phase == "prevolatile") %>%
    group_by(diagnosis, expected) %>%
    summarise(across(where(is.numeric), ~ mean(.x))) %>%
    group_by(diagnosis) %>%
    summarise(across(where(is.numeric), ~ diff(.x))) %>%
...
e1.1 = hypothesis(m.pal, 
                  "0 <   diagnosis1:expected1 + 
                       2*diagnosis2:expected1 + 
                         diagnosis3:expected1 + 
                         diagnosis1:expected1:phase1 + 
                       2*diagnosis2:expected1:phase1 + 
                         diagnosis3:expected1:phase1", 
                  alpha = 0.025)
e1.1

# prevolatile: COMP(unexpected - expected) != ADHD(unexpected - expected)
e1.2 = hypothesis(m.pal, 
                  "0 < 2*diagnosis1:expected1 + 
                         diagnosis2:expected1 + 
                         diagnosis3:expected1 + 
                       2*diagnosis1:expected1:phase1 + 
                         diagnosis2:expected1:phase1 + 
                         diagnosis3:expected1:phase1", 
                  alpha = 0.025)
e1.2

# prevolatile: COMP(unexpected - expected) != BOTH(unexpected - expected)
e1.3 = hypothesis(m.pal,  
                  "0 <   diagnosis1:expected1 + 
                         diagnosis2:expected1 + 
                       2*diagnosis3:expected1 + 
                         diagnosis1:expected1:phase1 + 
                         diagnosis2:expected1:phase1 + 
                       2*diagnosis3:expected1:phase1", 
                  alpha = 0.025)
e1.3
...
df.ms = df.ms %>%
  mutate(
    ...
    `h1c_COMP-ADHD unexpected-expected` = COMP_unexp_exp - ADHD_unexp_exp,
    ...
    `e12_prevolatile COMP-ADHD unexpected-expected` = 
      (COMP_pre_unexp - COMP_pre_exp) - (ADHD_pre_unexp - ADHD_pre_exp),
    `e13_prevolatile COMP-BOTH unexpected-expected` = 
...
Concerning reaction times, we neither found a credible difference in the effect of expectancy between the autistic and the comparison group (*estimate* = `r round(h1b$hypothesis$Estimate,2)` [`r round(h1b$hypothesis$CI.Lower,2)`, `r round(h1b$hypothesis$CI.Upper,2)`], *posterior probability* = `r round(h1b$hypothesis$Post.Prob*100,2)`%) nor between the ADHD and the comparison group (*estimate* = `r round(h1c$hypothesis$Estimate,2)` [`r round(h1c$hypothesis$CI.Lower,2)`, `r round(h1c$hypothesis$CI.Upper,2)`], *posterior probability* = `r round(h1c$hypothesis$Post.Prob*100,2)`%). We also did not observe a credible difference between the autistic and the comparison group regarding the difference between the prevolatile and the volatile phase (ASD-COMP: *estimate* = `r round(h1d$hypothesis$Estimate,2)` [`r round(h1d$hypothesis$CI.Lower,2)`, `r round(h1d$hypothesis$CI.Upper,2)`], *posterior probability* = `r round(h1d$hypothesis$Post.Prob*100,2)`%); therefore, our hypotheses are not supported by the data. Across groups, the task worked as expected: participants reacted faster on expected trials (unexpected-expected: *estimate* = `r round(e2.1$hypothesis$Estimate,2)` [`r round(e2.1$hypothesis$CI.Lower,2)`, `r round(e2.1$hypothesis$CI.Upper,2)`], *posterior probability* = `r round(e2.1$hypothesis$Post.Prob*100,2)`%) as well as on easier trials (easy-difficult: *estimate* = `r round(e2.6$hypothesis$Estimate,2)` [`r round(e2.6$hypothesis$CI.Lower,2)`, `r round(e2.6$hypothesis$CI.Upper,2)`], *posterior probability* = `r round(e2.6$hypothesis$Post.Prob*100,2)`%). 
...
df.pal %>%
  ...
  ggplot(aes(group, rt.cor, fill = expected, colour = expected)) + #
  geom_rain(rain.side = 'r',
boxplot.args = list(color = "black", outlier.shape = NA, show.legend = FALSE, alpha = .8),
violin.args = list(color = "black", outlier.shape = NA, alpha = .8),
boxplot.args.pos = list(
...
df.pal %>%
  ...
  group_by(subID, group, expected, phase) %>%
  summarise(
    rt.cor = median(rt.cor, na.rm = T)
  ) %>%
  ggplot(aes(group, rt.cor, fill = expected, colour = expected)) + #
...
```{r bf_rt2, fig.height=4}

# chose the top 4 models for closer inspection
...
kable(df.pal.bf.best %>% filter(priors == "chosen") %>% select(-priors) %>%
  filter(`population-level` != "1") %>% arrange(desc(bf.log)) %>%
        mutate(
          bf.log.diff = bf.log - lead(bf.log),
          bf.int = interpret_bf(bf.log.diff, log = T)
