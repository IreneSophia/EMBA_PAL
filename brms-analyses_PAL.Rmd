...
`r brms::bf(rt.cor ~ diagnosis * expected * phase * difficulty + 
                   (expected * phase * difficulty | subID) + 
                   (diagnosis * expected * phase * difficulty | trl))`
                   

`r brms::bf(rt.cor ~ diagnosis * expected * phase + 
                   (expected * phase | subID) + 
                   (diagnosis * expected * phase | trl))`
...
After having run the SBC, we realised that setting the threshold to Gaussian with a mean of 100ms does not fit well with a model based on aggregated reaction times - they are unlikely to be this low. Therefore, we change this one prior to a 200ms mean, but assume that the SBC still holds. 
...
e1.2 = hypothesis(m.pal, "0 < 2*(2*diagnosis1:expected1 + diagnosis2:expected1 + diagnosis3:expected1 +
                  2*diagnosis1:expected1:phase1 + diagnosis2:expected1:phase1 + diagnosis3:expected1:phase1)", alpha = 0.025)
e1.2
...
e1.3 = hypothesis(m.pal, "0 < 2*(diagnosis1:expected1 + diagnosis2:expected1 + 2*diagnosis3:expected1 +
                  diagnosis1:expected1:phase1 + diagnosis2:expected1:phase1 + 2*diagnosis3:expected1:phase1)", alpha = 0.025)
e1.3
...
df.ms = df.ms %>%
  mutate(
    ...
    e26_diffveasy     = rowMeans(across(matches(".*_difficult"))) - 
      rowMeans(across(matches(".*_easy")))
  )
